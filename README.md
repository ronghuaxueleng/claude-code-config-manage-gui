...
> **✨ 最新更新 (v1.7.0)**
> - 📦 **账号批量导入导出**: 支持选择性导出、URL 筛选、自定义保存位置
> - 🌐 **宿主机 IP 功能**: 账号切换时支持使用宿主机 IP
> - 🛡️ **CLAUDE.local.md 保护**: 切换账号时自动检测并备份现有配置文件
> - 🔧 **环境变量增强**: 自动添加隐私保护配置，拆分为独立环境变量
> - 📁 **Commands 目录支持**: 自动打包和释放 commands 目录下的文件
> - 🛠️ **Rust 安装脚本**: 国内镜像一键安装，支持多镜像源选择
> - 🐛 **Bug 修复**: 修复批量导入导出、token 匹配、数据库迁移等多个问题
> - ⚡ **CLI 增强**: 自动设置 onboarding、版本号同步、环境变量支持
